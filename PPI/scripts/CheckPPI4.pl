...
#
 ...
 unless (defined $file  && not defined $help && defined $nran && defined $pdb && $cutoff >= 2.0){
   printf "
          ...
          p    PDB of complex structure
          a    Sequence Alignment with Chain A  of PDB (Fasta format)
          b    Sequence Alignment with Chain B  of PDB (Fasta format)
  ...
  for $i (0..$#prta)
   {
     ...
     if ($prta[$i]->{atom} eq "CA" ){ push @seq1,substr($lode,$letter,1);push @pol1,substr($pode,$letter,1); $nresa++; }
   }
  ...
  for $i (0..$#prtb)
   {
     ...
     $letter=22;
     for $j (0..$#code){if ($code[$j] eq $res){$letter=$j;last;}}
     if ($prtb[$i]->{atom} eq "CA" ){ push @seq2,substr($lode,$letter,1);push @pol2,substr($pode,$letter,1); $nresb++; }
   }
...
# Calculate Z-scores
 ...
 if ( defined $output ) { open (OUTPUT,">$output"); select(OUTPUT);}
   ...
   if (defined $xml){
     ...
     printf " <protein>\n";
     printf "  <id>%s</id>\n",$pdbid;
     printf "  <length_a>%5d</length_a>\n",$#seqa+1;
     printf "  <length_b>%5d</length_b>\n",$#seqb+1;
     printf "  <global_energies>\n";
     printf "    <Epair>%10.3e</Epair>\n",$Pscore[0];
     printf "    <Ecomb>%10.3e</Ecomb>\n",$Energy[0];
     printf "    <Es3dc>%10.3e</Es3dc>\n",$Eaa3Denv[0];
     printf "    <Elocal>%10.3e</Elocal>\n",$Elocal[0];
     printf "    <E3dc>%10.3e</E3dc>\n",$E3Denv[0];
     printf "    <E3d>%10.3e</E3d>\n",$E3D[0];
     printf "    <ZEpair>%10.3e</ZEpair>\n",$Zscore;
     printf "    <ZEcomb>%10.3e</ZEcomb>\n",$ZEnergy;
     printf "    <ZEs3dc>%10.3e</ZEs3dc>\n",$ZEaa3Denv;
     printf "    <ZElocal>%10.3e</ZElocal>\n",$ZElocal;
     printf "    <ZE3dc>%10.3e</ZE3dc>\n",$ZE3Denv;
     printf "    <ZE3d>NA</ZE3d>\n";
     printf "    <Zene>%10.3e</Zene>\n",$ZZEnergy;
     printf "  </global_energies>\n";
     printf "  <residues>\n";
     for ($i=0;$i<=$#seqa;$i++){
     for ($k=0;$k<=$#seqb;$k++){
     if (abs($abEnergy[0][$i][$k])>1.0e-5 ||  abs($abPscore[0][$i][$k])>1.0e-5|| abs($abEaa3Denv[0][$i][$k])>1.0e-5|| abs($abE3Denv[0][$i][$k])>1.0e-5||abs($abElocal[0][$i][$k])>1.0e-5){
       printf "   <residue>\n";
       printf "     <number_x>%5d</number_x>\n",$i+1;
       printf "     <number_y>%5d</number_y>\n",$k+1;
       printf "     <type_x>%1s</type_x>\n",$seqaR[$tsa[$i]];
       printf "     <type_y>%1s</type_y>\n",$seqbR[$tsb[$k]];
       printf "     <Epair>%10.3e</Epair>\n",$abPscore[0][$i][$k];
       printf "     <Ecomb>%10.3e</Ecomb>\n",$abEnergy[0][$i][$k];
       printf "     <Es3dc>%10.3e</Es3dc>\n",$abEaa3Denv[0][$i][$k];
       printf "     <Elocal>%10.3e</Elocal>\n",$abElocal[0][$i][$k];
